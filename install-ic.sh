...
dfx deploy dex --argument '( record {
  canister_ids = record {
    ck_btc = principal "hewp7-sqaaa-aaaap-anxeq-cai";
    ck_usdt = principal "hnved-eyaaa-aaaap-anxfa-cai";
  };
})' --ic --mode=reinstall --yes
...
dfx deploy protocol --argument '( variant { 
  init = record {
    canister_ids = record {
      supply_ledger = principal "hnved-eyaaa-aaaap-anxfa-cai";
      collateral_ledger = principal "hewp7-sqaaa-aaaap-anxeq-cai";
    ...
    parameters = record {
      ...
      clock = variant { REAL };
      twap_config = record {
        window_duration = variant { MINUTES = 30 };
...
dfx canister call minter mint_btc '( record {
  ...
  to = record {
    owner = principal "ena4g-5yaaa-aaaap-an3ca-cai";
    subaccount = null;
  };
})' --ic
