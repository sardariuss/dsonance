...
import { LOCK_EMOJI, DSONANCE_COIN_SYMBOL, CONSENT_EMOJI, TIMESTAMP_EMOJI, DISSENT_EMOJI, MINING_EMOJI } from "../../constants";
import { get_current, map_timeline, map_timeline_hack, to_number_timeline } from "../../utils/timeline";
...
export const DesktopBallotDetails : React.FC<BallotDetailsProps> = ({ ballot, now, releaseTimestamp }) => {
    ...
    return (
        <div className="grid grid-cols-2 gap-x-2 gap-y-2 justify-items-center w-full p-2">

          <div className="flex flex-row space-x-1 items-baseline justify-center w-full py-1 rounded-lg shadow-sm bg-slate-200 dark:bg-gray-800">
          ...
          <div className="flex flex-row space-x-1 items-baseline justify-center w-full py-1 rounded-lg p-3 shadow-sm bg-slate-200 dark:bg-gray-800">
          ...
          <div className="flex flex-col items-center justify-center space-x-1 w-full py-1 rounded-lg p-3 shadow-sm bg-slate-200 dark:bg-gray-800">
            <div className="flex flex-row space-x-1 items-baseline">
              ...
              { duration_diff !== undefined && <span>{`(+ ${formatDuration(duration_diff)})`}</span> }
            </div>
            <DurationChart 
          ...
          <div className="flex flex-col items-center justify-center space-x-1 w-full py-1 rounded-lg p-3 shadow-sm bg-slate-200 dark:bg-gray-800">
            ...
            <div className="flex flex-row space-x-1 items-baseline">
              ...
              { consent_diff !== undefined && <span>{`(${consent_diff.toFixed(3)})`}</span> }
            </div>
            <DurationChart 
          ...
          <div className="flex flex-col items-center justify-center space-x-1 w-full py-1 rounded-lg p-3 shadow-sm bg-slate-200 dark:bg-gray-800">
            <div className="flex flex-row space-x-1 items-baseline">
              ...
              <span className="italic text-gray-600 dark:text-gray-400 text-sm">Mining earned:</span>
          ...
          <div className="flex flex-col items-center justify-center space-x-1 w-full py-1 rounded-lg p-3 shadow-sm bg-slate-200 dark:bg-gray-800">
            <div className="flex flex-row space-x-1 items-center">
              ...
              { apr_diff !== undefined && <span>{`(${apr_diff.toFixed(2)}%)`}</span> }
            </div>
            <DurationChart 
              duration_timelines={ new Map([
        ...
        <div className="flex flex-col justify-items-center w-full mt-2 space-y-1">
          ...
          >
            <div className="grid grid-cols-9 items-center text-center justify-center w-full pl-2 pr-10">
                ...
                <span className="flex flex-row space-x-1 col-span-4">
                    <span>{MINING_EMOJI}</span>
                    <span className="italic text-gray-600 dark:text-gray-400 text-sm">Mining reward:</span> 
                </span>
                <span className="col-span-4 justify-self-end">
          ...
          >
            <div className="grid grid-cols-9 items-center text-center justify-center w-full pl-2 pr-10">
                ...
                <span className="flex flex-row space-x-1 col-span-4 items-center">
                  <BitcoinIcon/>
                  <span className="italic text-gray-600 dark:text-gray-400 text-sm">APR:</span> 
                </span>
                <span className="col-span-4 justify-self-end">
                  {ballot.YES_NO.foresight.current.data.apr.current.toFixed(2) + "%" }
                </span>
            </div>
