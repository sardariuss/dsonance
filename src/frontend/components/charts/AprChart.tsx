...
const AprChart: React.FC<AprChartProps> = ({
  ...
  // Process timeline data to extract APR history
  const generateChartData = () => {
    const supplyAprPoints = [];
    const borrowAprPoints = [];
    ...
    for (const entry of lendingIndexTimeline.history) {
      ...
      borrowAprPoints.push({
        x: hours,
        y: entry.data.borrow_rate * 100 // Convert to percentage
      });
    }
    ...
    borrowAprPoints.push({
      x: currentHours,
      y: lendingIndexTimeline.current.data.borrow_rate * 100
    });

    // Calculate max hours for x-axis
    const maxHours = Math.max(...[...supplyAprPoints, ...borrowAprPoints].map(p => p.x));
  ...
  return (
    <div className="h-80">
      <ResponsiveLine
        ...
        colors={[
          theme === 'dark' ? '#60a5fa' : '#3b82f6',  // Supply APR - blue
          theme === 'dark' ? '#f59e0b' : '#f97316'   // Borrow APR - orange
        ]}
        lineWidth={3}
        enablePoints={data[0].data.length < 50} // Only show points if not too many
        pointSize={6}
        pointBorderWidth={2}
