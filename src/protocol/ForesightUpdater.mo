...
module {
    ...
    public class ForesightUpdater({
        ...
        get_yield: () -> { earned: Float; apr: Float; tvl: Nat; };
    }) {

        public func update_foresights({ time: Nat; }) {
            let foresights = compute_foresights(ballots, time);
            ...
            Map.map<UUID, YesNoBallot, Foresight>(ballots, Map.thash, func(id: Text, ballot: YesNoBallot) : Foresight {
                ...
                let actual_reward = do {
                    if(sum_contribs.cumulated <= 0) {
                        ...
                        (ballot_contrib.cumulated / sum_contribs.cumulated) * earned;
                    };
                };
                ...
                let projected_reward = do {
                    if(sum_contribs.current <= 0) {
                        ...
                        (ballot_contrib.current / sum_contribs.current) * apr * 100 * Float.fromInt(tvl) * remaining_duration;
                    };
                };

                let reward = Int.abs(Float.toInt(actual_reward + projected_reward));
                
                let ballot_apr = (100 * Float.fromInt(reward) / Float.fromInt(ballot.amount)) / (lock_duration);
                {
                    reward;
                    apr = {
                        current = ballot_apr;
                        potential = ballot_apr / Timeline.current(ballot.consent);
                    };
                };
            });
        };
